...
# ==================== THIRD-PARTY HAWB (Staff Only) ====================
@login_required(login_url='login')
@require_http_methods(["POST"])
def create_third_party_hawb(request):
    """
    Manually register a HAWB number generated by a third party (e.g. another
    carrier). Only the AWB number is required; once created it behaves like
    any other booked shipment (scannable, trackable, can be bagged/manifested).
    """
    if not request.user.is_staff:
    ...
    try:
        ...
        awb_number = (data.get('awb_number') or '').strip()
        if not awb_number:
            return JsonResponse({
                'success': False,
        ...
        if Shipment.objects.filter(awb_number=awb_number).exists():
            return JsonResponse({
                'success': False,
                'error': f'AWB number {awb_number} already exists in the system'
            }, status=400)
        ...
        shipment = Shipment.objects.create(
            ...
            source=Shipment.SOURCE_THIRD_PARTY,
            direction=data.get('direction') or None,
            customer=customer,
            declared_value=data.get('declared_value') or None,
            declared_currency=data.get('declared_currency', 'USD'),
            weight_estimated=data.get('weight_estimated') or None,
            ...
            is_liquid=data.get('is_liquid', False),
            is_cod=data.get('is_cod', False),
            cod_amount=data.get('cod_amount') or None,
            special_instructions=data.get('special_instructions', ''),
        ...
        return JsonResponse({
            'success': True,
            'message': f'Third-party HAWB {shipment.awb_number} added successfully',
            'parcel_id': shipment.id,
            'awb_number': shipment.awb_number,
        })
